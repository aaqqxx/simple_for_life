    ...
    for div in alldiv:
        ...
        for ul in allul:
            ...
            for li in allli:
                ...
                print i, filename.decode('gbk').encode('utf-8')
...
def getSongUrl(songName, authorName):
    ...
    songUrl = 'http://box.zhangmen.baidu.com/x?op=12&count=1&mtype=1&title=%s$$%s$$$$&url=&listenreelect=0&.r=0.1696378872729838' % (
        urllib.quote(songName.encode('gbk')), urllib.quote(authorName.encode('gbk')))
    ...
    try:
        # print url1[0][:url1[0].rindex('/') + 1] + url2[0]
