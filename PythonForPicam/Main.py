...
def main():
    ...
    distribution = piint()
    release = piint()
    print 'Check Software Version. ', Picam_GetVersion(pointer(major), pointer(minor), pointer(distribution),
                                                       pointer(release))
    ...
    print "Getting readout stride. ", Picam_GetParameterIntegerValue(camera, ctypes.c_int(PicamParameter_ReadoutStride),
...
if __name__ == '__main__':
    ...
    ## Test Routine to connect a demo camera
    ## p23
    print 'Preparing to connect Camera'
    model = ctypes.c_int(10)
    serial_number = ctypes.c_char_p('Cam 1')
    PicamID = PicamCameraID()
