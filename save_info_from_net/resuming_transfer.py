...
def test():
    ...
    req.add_header('Range', 'bytes=20-100')  # set the range, from 0byte to 19byte, 20bytes len
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', type=int, default=defaults['thread_count'], dest="thread_count",
                        help='thread counts to downloading')
    ...
    argv = sys.argv[1:]
    if len(argv) == 0:
        argv = ['http://download.virtualbox.org/virtualbox/5.0.2/Oracle_VM_VirtualBox_Extension_Pack-5.0.2-102096.vbox-extpack']
